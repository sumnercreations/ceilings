...
namespace :deploy do
  ...
  task :ng_build do
    on roles(:production) do
      ...
      execute "cd #{release_path} && ng build --environment=prod --target=prod --no-progress"
    end

    on roles(:staging) do
      print "building in staging environment"
      execute "cd #{release_path} && ng build --environment=staging --target=staging --no-progress"
    end
  end
